class Validator {
  ...
  validade(form) {
    ...
    //limpa todas as dalidações antigas
  ...
  minlength(input, minValue) {
    ...
    let errorMenssage = `O campo precisa ter pelo menos ${minValue} caracacteres`;
  ...
  // verificar se um campo está iqual o outro
  equal(input, inputName) {
    let inputTocompare = document.getElementByName(inputName)[0];

    let errorMessage = `Este campo precisa estr iqual ao ${inputName}`;
  ...
  passwordvalidate(input) {
    ...
    for(let i = 0; charArr.length > i; i++) {
      if(charArr[i] === charArr[i].toUpperCase() && isNaN(parseInt(charArr))) {
        uppercases++;
